...
![Wigner-Seitz Cell](img/wigner.png)
...
![Simple Cubic](img/sc.png)

#### Face Centered Cubic (FCC) lattice

Another important lattice similar to the SC-lattice but with additional lattice points at each face of the cube.
The lattice vectors are $\vec{a_1} = (\frac{a}{2}, \frac{a}{2}, 0)$, $\vec{a_2} = (\frac{a}{2}, 0, \frac{a}{2})$ and $\vec{a_3} = (0, \frac{a}{2}, \frac{a}{2})$.

![Face Centered Cubic](img/fcc.png)

#### Body Centered Cubic (BCC) lattice

Lastly there is the SC-lattice but with one additional lattice point at the center of the cube.
The lattice vectors are $\vec{a_1} = (a, 0, 0)$, $\vec{a_2} = (0, a, 0)$ and $\vec{a_3} = (\frac{a}{2}, \frac{a}{2}, \frac{a}{2})$.

![Body Centered Cubic](img/bcc.png)
...
![Miller Indices Intersections](img/miller_g.png)

The plane then intercepts the axes at $\frac{\vec{a}}{h}$, $\frac{\vec{b}}{k}$ and $\frac{\vec{c}}{l}$ and does not intercept an axis at all if the corresponding index is $0$.
...
as $\vec{d}$, $\vec{a}$ and the plane create a right triangle. This is of course true for all lattice vectors. After multiplying by $2\pi$ we get the following relation

$$\vec{a} \frac{2 \pi \vec{d}}{d^2} = 2 \pi h$$
$$\vec{b} \frac{2 \pi \vec{d}}{d^2} = 2 \pi k$$
$$\vec{c} \frac{2 \pi \vec{d}}{d^2} = 2 \pi l$$

This for now seems kind of arbitrary but we will need it later.

![Miller Planes Examples](img/miller_planes.jpg)
...
|---|---|
| **SC** (side $a$) | **SC** (side $\frac{2\pi}{a}$) |
| **FCC** (side $a$) | **BCC** (side $\frac{4\pi}{a}$) |
| **BCC** (side $a$) | **FCC** (side $\frac{4\pi}{a}$) |
...
::: {.callout-important title="Laue Conditions"}
...
$$\vec{G}_{h,k,l} \cdot \vec{a} = 2 \pi h$$
$$\vec{G}_{h,k,l} \cdot \vec{b} = 2 \pi k$$  
$$\vec{G}_{h,k,l} \cdot \vec{c} = 2 \pi l$$
...
$$\vec{G}_{h,k,l} = \frac{2 \pi \vec{d}}{d^2}$$

This means $\vec{G}_{h,k,l}$ is **parallel to $\vec{d}$** and **perpendicular to the $h,k,l$-plane**.
...
$$A_{tot} = \sum_n e^{-i(\vec{K} \cdot \vec{r_n})} f_n = \sum_l e^{-i(\vec{K} \cdot \vec{r_l})} \sum_p e^{-i(\vec{K} \cdot \vec{r_p})}f_n$$

We call

$$S = \sum_p e^{-i(\vec{K} \cdot \vec{r_p})}f_p$$
