...
![Piston](img/piston.png)

In a formula you can write the first law of Thermodynamics like this

::: {.callout-important title="First law of Thermodynamics"}
$$dU = dQ + dW$$
...
where $f$ is the degree of freedom for the gas, $N$ the number of gas particles, $k_B$ the Boltzmann constant and $T$ the temperature of the gas.
...
::: {.callout-important title="The Ideal Gas Law"}
$$p V = N k_B T$$
...
$$\left(\frac{f}{2}+1\right) p dV = -\frac{f}{2} V dp$$
...
::: {.callout-important title="Adiabatic Relation for Pressure and Volume"}
$$p V^\gamma = \text{const}$$
...
$$N k_B T V^{-1} V^\gamma = \text{const}$$

::: {.callout-important title="Adiabatic Relation for Temperature and Volume"}
...
![Adiabatic Heating](img/adiabatic-heating.png)
...
E.g. The 2019 Ford Super Duty has a compression ratio of $16.2:1$. We derived that $T V^{\gamma-1} = \text{const}$ and so
...
$$T_2 = \left(\frac{16.2}{1}\right)^{\frac{2}{5}} 300 K \approx 914 K = 641°C$$
